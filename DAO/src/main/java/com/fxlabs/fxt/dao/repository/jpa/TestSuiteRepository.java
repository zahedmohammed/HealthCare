...
public interface TestSuiteRepository extends JpaRepository<TestSuite, String> {
    ...
    @Query("SELECT " +
            "    new com.fxlabs.fxt.dao.entity.project.TestSuiteCount(ts.parent, ts.method , count(ts)) " +
            "FROM " +
            "    TestSuite ts " +
            "WHERE " +
    ...
    @Query("SELECT " +
            "    new com.fxlabs.fxt.dao.entity.project.TestSuiteCount(ts.parent, ts.category , count(ts)) " +
            "FROM " +
            "    TestSuite ts " +
            "WHERE " +
            "    ts.project.id = ?1 AND ts.autoGenerated = ?2 AND ts.inactive = false " +
            "GROUP BY " +
            "    ts.parent, ts.category")
    List<TestSuiteCount> countByCategoryAndAutoGen(String project, boolean auto);

    @Query("SELECT " +
            "    new com.fxlabs.fxt.dao.entity.project.TestSuiteCount(ts.parent, ts.severity , count(ts)) " +
            "FROM " +
            "    TestSuite ts " +
            "WHERE " +
            "    ts.project.id = ?1 AND ts.autoGenerated = ?2 AND ts.inactive = false " +
            "GROUP BY " +
            "    ts.parent, ts.severity")
